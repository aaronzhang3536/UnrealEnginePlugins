...
struct FAssetRuleTreeItem : public ISlateMetaData, TSharedFromThis<FAssetRuleTreeItem>
{
	SLATE_METADATA_TYPE(FAssetRuleTreeItem, ISlateMetaData)
	FString AssetType;
	FAssetRuleItem* RuleItem;
	bool bIsTypeRuleItem = false;
	TArray<TSharedPtr<FAssetRuleTreeItem>> Children;
	class SErdanAssetManagerView* OwnedManagerView;

public:
	bool CanInteract() const
	{
		return !bIsTypeRuleItem;
	}
};

typedef TSharedPtr<FAssetRuleTreeItem> FAssetRuleTreeItemPtr; 
typedef TSharedRef<FAssetRuleTreeItem> FAssetRuleTreeItemRef;

class SErdanAssetManagerRuleTreeView : public STreeView< FAssetRuleTreeItemPtr >
{
public:
		
	/** Construct this widget */
	void Construct(const FArguments& InArgs, TSharedRef<SErdanAssetManagerView> Owner);

	//void FlashHighlightOnItem( FAssetRuleTreeItemPtr FlashHighlightOnItem );

	const TWeakPtr<SErdanAssetManagerView>& GetAssetManagerViewPtr() { return SceneOutlinerWeak; }
protected:
...
private:
	...
	TWeakPtr<SErdanAssetManagerView> SceneOutlinerWeak;
};

/** Widget that represents a row in the outliner's tree control.  Generates widgets for each column on demand. */
class SAssetRuleTreeRow
	: public SMultiColumnTableRow< FAssetRuleTreeItemPtr >
{

public:

	SLATE_BEGIN_ARGS(SAssetRuleTreeRow) {}

		/** The list item for this row */
		SLATE_ARGUMENT(FAssetRuleTreeItemPtr, Item )

	SLATE_END_ARGS()


	/** Construct function for this widget */
	void Construct( const FArguments& InArgs, const TSharedRef<SErdanAssetManagerRuleTreeView>& RuleTreeView, TSharedRef<SErdanAssetManagerView> AssetManagerView);
...
private:
	...
	TWeakPtr< SErdanAssetManagerView > AssetManagerViewWeak;

	/** The item associated with this row of data */
	TWeakPtr<FAssetRuleTreeItem> Item;
