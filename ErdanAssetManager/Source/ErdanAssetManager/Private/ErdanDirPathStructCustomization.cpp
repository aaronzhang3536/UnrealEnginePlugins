...
TSharedRef<IPropertyTypeCustomization> FErdanDirPathStructCustomization::MakeInstance()
{
	return MakeShareable(new FErdanDirPathStructCustomization());
}

void FErdanDirPathStructCustomization::CustomizeHeader( TSharedRef<IPropertyHandle> StructPropertyHandle, class FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& StructCustomizationUtils )
{
	...
	if(PathProperty.IsValid())
	{
		...
		if(bContentDir)
		{
			...
			.OnClicked( this, &FErdanDirPathStructCustomization::OnPickContent, PathProperty.ToSharedRef() )
			.ContentPadding(2.0f)
			.ForegroundColor( FSlateColor::UseForeground() )
			.IsFocusable(false)
			.IsEnabled(this, &FErdanDirPathStructCustomization::IsBrowseEnabled, StructPropertyHandle)
			[
				SNew(SImage)
		...
		else
		{
			...
			.OnClicked( this, &FErdanDirPathStructCustomization::OnPickDirectory, PathProperty.ToSharedRef(), bRelativeToGameContentDir, bUseRelativePath )
			.ContentPadding( 2.0f )
			.ForegroundColor( FSlateColor::UseForeground() )
			.IsFocusable( false )
			.IsEnabled( this, &FErdanDirPathStructCustomization::IsBrowseEnabled, StructPropertyHandle )
...
void FErdanDirPathStructCustomization::CustomizeChildren( TSharedRef<IPropertyHandle> StructPropertyHandle, class IDetailChildrenBuilder& StructBuilder, IPropertyTypeCustomizationUtils& StructCustomizationUtils )
{
}

FReply FErdanDirPathStructCustomization::OnPickContent(TSharedRef<IPropertyHandle> PropertyHandle) 
{
	FContentBrowserModule& ContentBrowserModule = FModuleManager::LoadModuleChecked<FContentBrowserModule>("ContentBrowser");
	FPathPickerConfig PathPickerConfig;
	PropertyHandle->GetValue(PathPickerConfig.DefaultPath);
	PathPickerConfig.bAllowContextMenu = false;
	PathPickerConfig.OnPathSelected = FOnPathSelected::CreateSP(this, &FErdanDirPathStructCustomization::OnPathPicked, PropertyHandle);
...
FReply FErdanDirPathStructCustomization::OnPickDirectory(TSharedRef<IPropertyHandle> PropertyHandle, const bool bRelativeToGameContentDir, const bool bUseRelativePath) const
{
	FString Directory;
...
bool FErdanDirPathStructCustomization::IsValidPath(const FString& AbsolutePath, const bool bRelativeToGameContentDir, FText* const OutReason) const
{
	if(bRelativeToGameContentDir)
	{
...
void FErdanDirPathStructCustomization::OnPathPicked(const FString& Path, TSharedRef<IPropertyHandle> PropertyHandle)
{
...
bool FErdanDirPathStructCustomization::IsBrowseEnabled(TSharedRef<IPropertyHandle> PropertyHandle) const
{
